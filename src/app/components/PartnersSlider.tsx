...
const getPartnersSliderData = async (): Promise<PartnersSliderData | null> => {
    return client.fetch(`
        *[_type == "partnersSlider"][0] {
            images[]{
                asset->{
                    url
                },
...
const PartnersSlider: React.FC = () => {
    const [sliderData, setSliderData] = React.useState<PartnersSliderData | null>(null);
    const [loading, setLoading] = React.useState(true);

    React.useEffect(() => {
        const fetchData = async () => {
            const data = await getPartnersSliderData();
            setSliderData(data);
            setLoading(false);
        };
        fetchData();
    ...
    return (
        <div className="w-full mb-10 max-w-[1350px] mx-auto px-4">
            <Carousel
                opts={{
                    align: "start",
                }}
